...
(function() {
	...
	home.on('click', function() {
		location.reload();
		/*
		search_internet.removeClass('hide');
		search_internet.addClass('show');		
		dailySummary.removeClass('hide');
		dailySummary.addClass('show');
	...
	var Foods = Backbone.Collection.extend({  
	    model: Food,  //model 'Food'is specified for a collection by overriding the 'model' property        
		// Implement localstorage
		localStorage: new Backbone.LocalStorage('Health-Tracker')
	});
	//The collection 'my_foods' is instantiated by using new keyword    
	my_foods = new Foods;
	...
	query_btn.on('click', function() {
		...
		$.ajax({
			...
			if (nu < 1) {
				//alert("Sorry there is no result for this search. Please refine your search");
				add_food_manually.removeClass('hide');
				add_food_manually.addClass('show');
				return;
			}
			...
			$('#api_data').html(items);  //display results
			$('#query').val(''); //reset query input
	...
	save_nd_eat_food.on('click', function() {
		...
		if (!date || !name || !calories) {
			//console.log(date, time, calories)//keep for troubleshooting
			return;
		}
...
(function() {	
	...
	//var time = moment().format('LLL');  // take note, don't use
